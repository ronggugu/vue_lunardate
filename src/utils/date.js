...
var monthArrayChinese = new Array("一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "十二月");
...
function calendar(that, y, m) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    lDObj.dayCyl = parseInt(lDObj.dayCyl)
    that[i] = new calElement(y, m + 1, i + 1, nStr1[(i + firstWeek) % 7],
      lY, lM, lD++, lL,
  ...
  that[tmp2].solarTerms = solarTerm[m * 2 + 1]
